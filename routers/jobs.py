...
def create_job(job_data: schemas.JobCreate, db: Session = Depends(get_db), current_user=Depends(require_employer)):
...
def update_job(job_id: int, job_data: schemas.JobUpdate, db: Session = Depends(get_db), current_user=Depends(require_employer)):
    job = db.query(models.Job).filter(models.Job.id == job_id).first()
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
...
def delete_job(job_id: int, db: Session = Depends(get_db), current_user=Depends(require_employer)):
    job = db.query(models.Job).filter(models.Job.id == job_id).first()
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    if job.employer_id != current_user.id:
